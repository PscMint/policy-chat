...
import { bing_search, chat, chatfile, getOpts } from '@/api/chat'
...
const searchOptions = computed(() => {
  ...
  let promList = prompt.value.split(' ')
  const res = promptTemplate.value.filter((item:{key:string})=>{
    for(let str of promList){
...
onBeforeMount(async ()=>{
  ...
  promptStore.updatePromptList(promptLst)
})
onMounted(() => {
  scrollToBottom()
